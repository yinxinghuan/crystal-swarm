...
- 主体为 2000 个 `DodecahedronGeometry(5, 0)` 实例，随机分布在 x/y/z 均为 -115 到 115 的 230 world units 立方空间。
- 每个实例缩放为 0.2 到 1.0，材质使用 `MeshPhysicalMaterial`，`roughness=0.28`、`metalness=0.02`、`transmission=0.12`、`thickness=5`、`opacity=0.92`、`emissiveIntensity=0.22`。
...
- 素材清单：`public/img/aigram.svg` 使用新版 AlterU 单色竖向 mark，作为右下角 28px 水印；`public/poster.png` 是通过平台生图接口直接生成带标题的 1024 × 1024 方形海报，仅做内裁去除边缘装饰，画面本身不带圆角；其余游戏内视觉由 Three.js 几何、材质、灯光和后处理生成。
