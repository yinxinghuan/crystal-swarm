...
- `public/poster.png`：1024 × 1024 平台封面图，由平台 gen-image 直接文生图生成标题和 key art，最终 PNG 不内置圆角。
- `gen_poster.py`：调用平台 gen-image 接口生成带标题海报，执行方形内裁，并同步写入游戏 repo 与 `games/posters/crystal-swarm.png`。
...
- 加玩法目标：可在 `animate()` 中增加目标区域命中检测，或在 pointer 交互里累计分数；不要让规则层遮挡材质和群体运动的主视觉。
- 换海报：运行 `gen_poster.py` 重新生成 `public/poster.png` 和 `games/posters/crystal-swarm.png`；海报图形本身保持直角，不在 PNG 中内置圆角。
